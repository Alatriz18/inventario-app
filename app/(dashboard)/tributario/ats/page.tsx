  ...
  const generarXML = async () => {
    ...
    try {
      ...
      comprasMes.forEach(f => {
        ...
        const conceptosRenta = Array.from(gruposRenta.values());
        ...
        // El bloque air es obligatorio siempre; si no hubo retención de renta se
        // reporta con el código 332 (otras compras no sujetas a retención) en 0.
        const airNode = det.ele('air');
        if (conceptosRenta.length > 0) {
          conceptosRenta.forEach(c => {
            const da = airNode.ele('detalleAir');
            da.ele('codRetAir').txt(c.codigo);
            da.ele('baseImpAir').txt(c.base.toFixed(2));
            da.ele('porcentajeAir').txt(String(c.porcentaje));
            da.ele('valRetAir').txt(c.valor.toFixed(2));
          });
        } else {
          const da = airNode.ele('detalleAir');
      ...
      const ventasEstabNode = doc.ele('ventasEstablecimiento');
      const ventaEst = ventasEstabNode.ele('ventaEst');
      ventaEst.ele('codEstab').txt(config.establecimiento.padStart(3,'0'));
      ventaEst.ele('ventasEstab').txt(resumen.totalVentas.toFixed(2));
      ventaEst.ele('ivaComp').txt('0.00');
